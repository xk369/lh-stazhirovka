...
- Queue assignment flow: trainees do not see public free dates and can only enter `queue`. Server sets `application.queueJoinedAt` when an application newly enters active queue; recruiter queue cards show this time/wait duration and sort within priority groups from earlier to later queue entry. Recruiter sends a 1-hour assignment confirmation request from the queue; trainee `Да` moves the application to `confirmed`, `Нет` keeps it in `queue`, and timeout moves it to `queue_expired` so the active queue seat hold is released. A trainee can withdraw back to `queue` before or after workgroup invite; `RECRUITER_WITHDRAWAL_CHAT_ID` receives a service notification.
